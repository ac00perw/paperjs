        ...
        buildMatrix() {
            for (var j = 0, yc = Math.ceil(this.height / this.boxSize.y); j < yc; j++) { //build rows
                for (var i = 0, c = Math.ceil(this.width / this.boxSize.x); i < c; i++) {
                    console.log(i%2);
                    this.matrix.push({ x: i * this.boxSize.x + this.rand(0,2)+((j % 2) * (this.boxSize.x/2)), y: (j * this.boxSize.y + (j*3))});
                }
            }
        ...
        drawShapes(count) {
            ...
            if(vm.matrix.length > 0){
                ...
                // var point = new paper.Point(vm.width / vm.points * i, vm.height/2);
                    ...
                    vm.shapes[vm.i] = new paper.Path.Rectangle(new paper.Point(spot), new paper.Size(vm.boxSize.x, vm.boxSize.y));
                    ...
                    vm.shapes[vm.i].opacity = vm.rand(.4,1);
